...
It then also runs the same full update check Unraid's own "Check for Updates" button
triggers (`dockerupdate check`), so the badge clears immediately instead of waiting for
Unraid's next scheduled check. A more targeted per-image reload
(`DockerUpdate::reloadUpdateStatus()`) does refresh the JSON entry on its own, but
empirically the Docker tab's badge doesn't pick that up until the full check runs — only
the full check actually clears it, so that's what gets used here. Both steps are
best-effort: if either fails (e.g. on an Unraid version where the path or script has
